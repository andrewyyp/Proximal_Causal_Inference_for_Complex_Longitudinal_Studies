para_set <- list(mu_X0 = -0.35,
                 ...
                 mu_U0 = 0.35,
                 sigma_U0 = 0.5,
                 alpha_A0 = c(-0.5, -0.35, 0.35),
                 mu_Z0 = c(0.2, 0.5, 0.5, 0.75),
                 sigma_Z0 = 0.5,
                 mu_W0 = c(0.2, 0.5, -0.95),
                 sigma_W0 = 0.5,
                 mu_X1 = c(0.2, 0.7, 0.7, 0),
                 sigma_X1 = 0.5,
                 mu_U1 = c(0.2, 0.7, 0, 0.7),
                 sigma_U1 = 0.5,
                 alpha_A1 = c(-0.5, 0.5, -0.35, 0.35, -0.35, 0.35),
                 mu_Z1 = c(0.2, 0, 0.5, 0.5, -0.75, 0.5, 0.5, -0.75),
                 sigma_Z1 = 0.5,
                 mu_W1 = c(0.35, 0, 0.45, -0.85, 0.45, -0.85),
                 sigma_W1 = 0.5,
                 mu_Y = c(-1.3, 1, 1.14, 0, 0, 0, 0.5, -0.7, 0.2, -0.7),
                 sigma_Y = 0.5
)
...
data_gen <- function(N, para_set) {
  ...
  # generate A0
  prop_score_0 <- 1/(1 + exp(-cbind(1, X0, U0) %*% para_set$alpha_A0))
  A0 <- rbinom(N, 1, prop_score_0)
  
  # generate Z0
  Z0 <- cbind(1, A0, X0, U0) %*% para_set$mu_Z0 + rnorm(N, 0, para_set$sigma_Z0)
  
  # generate W0
  W0 <- cbind(1, X0, U0) %*% para_set$mu_W0 + rnorm(N, 0, para_set$sigma_W0)
  ...
  # generate A1
  prop_score_1 <- 1/(1 + exp(-cbind(1, A0, X0, U0, X1, U1) %*% para_set$alpha_A1))
  A1 <- rbinom(N, 1, prop_score_1)
  
  
  # generate Z0
  Z1 <- cbind(1, Z0, A1, X1, U1, A0, X0, U0) %*% para_set$mu_Z1 + rnorm(N, 0, para_set$sigma_Z1)
  
  
  # generate W0
  W1 <- cbind(1, W0, X1, U1, X0, U0) %*% para_set$mu_W1 + rnorm(N, 0, para_set$sigma_W1)
  
  #generate Y
  Y <- cbind(1, A1, A0, A1 * A0, W1, W0, X1, U1, X0, U0) %*% para_set$mu_Y + rnorm(N, 0, para_set$sigma_Y)
